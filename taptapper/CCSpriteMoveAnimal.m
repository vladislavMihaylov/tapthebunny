...
- (void) moveAction // название - пиздос =)
{
    ...
    [self runAction: [CCSequence actions:
                                    [CCMoveTo actionWithDuration: (moveTime - [Settings sharedSettings].gameMode * 0.07)
                                                        position: ccp(self.position.x, self.position.y + height)],
                                    [CCCallBlock actionWithBlock: ^(id sender) {self.isCanTap = YES;} ],
                                    [CCDelayTime actionWithDuration: (delayTime - [Settings sharedSettings].gameMode * 0.05)],
                                    [CCCallBlock actionWithBlock: ^(id sender) {self.isCanTap = NO;} ],
                                    [CCMoveTo actionWithDuration: (0.5 - [Settings sharedSettings].gameMode * 0.05)
